...
function retrieveDummyData() {
  var patientdata = {
	...
	"telcos": ["Phone plan", "Edge", "Extra Plus", "5G", "90Gb data", "4G LTE data", "Text and Talk", "1 phone line"]
	"finances": ["Financial planning", "Best practices", "Analysis", "Financial processes"]
  }
  fillUI(patientdata);
}
...
function fillUI(patientdata) {
  ...
  patientdata.finances.forEach(function(finance) {
    var box = document.createElement('div');
    box.className = 'boxitem';
    box.innerHTML = '<img class="beaker" src="/images/beaker.svg"><div class="boxitemlabel">' + finance + '</div>'
    finances.appendChild(box);
  })

    var telcos = document.getElementById('telco');

  patientdata.telcos.forEach(function(telco) {
    var box = document.createElement('div');
    box.className = 'boxitem';
    box.innerHTML = '<img class="beaker" src="/images/beaker.svg"><div class="boxitemlabel">' + telco + '</div>'
    telcos.appendChild(box);
  })
